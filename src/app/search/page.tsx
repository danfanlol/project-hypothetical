...
export default function SearchPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 w-full flex flex-col gap-6">
      <div>
        <h1 className="text-xl font-semibold text-zinc-900 mb-1">Search by Position</h1>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="w-full max-w-[360px] shrink-0 flex flex-col gap-3">
          <div className="flex flex-col gap-1">
            <label className="text-sm font-medium text-zinc-700">Position FEN</label>
            <input
              type="text"
              value={fenInput}
          ...
          {normalizedFen ? (
            ...
            <div className="w-full aspect-square bg-zinc-50 border-2 border-dashed border-zinc-300 rounded-md flex items-center justify-center">
              <p className="text-zinc-400 text-sm">Board preview</p>
            </div>
          )}
        </div>
        ...
        <div className="flex-1 flex flex-col gap-4">
          ...
          {hasInput && loaded && !isInvalid && matches.length === 0 && (
            <div className="flex flex-col items-center justify-center gap-2 h-full min-h-[200px] text-center">
              <p className="text-zinc-500 font-medium">No match found</p>
              <p className="text-zinc-400 text-sm">
                No saved lines reach this position after their two setup moves.
              </p>
              <Link
                href="/create"
                className="mt-2 px-4 py-2 text-sm bg-zinc-900 text-white rounded-md hover:bg-zinc-700 transition-colors"
              >
                + Add this position
              </Link>
            </div>
            ...
            <>
              ...
              <ul className="flex flex-col gap-3">
                {matches.map((p) => (
                  <li
                    key={p.id}
                    className="border border-zinc-200 rounded-lg bg-white px-4 py-4 flex flex-col gap-3"
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex flex-col gap-0.5 min-w-0">
                        ...
                        <p className="text-xs text-zinc-400 font-mono truncate">{p.fen}</p>
                      </div>
                    ...
                    <div className="flex items-center gap-1.5 text-sm">
                      ...
                      <span className="font-mono text-zinc-500">{p.move2}</span>
                      <span className="text-zinc-300">→</span>
